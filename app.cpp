...
inline void Kinect::drawBody()
{
    ...
    Concurrency::parallel_for( 0, BODY_COUNT, [&]( const int count ){
		...
		string timeStr;
		char buffer[256];
		sprintf(buffer, "%d-%2d-%2d %02d:%02d:%02d.%03d", sys.wYear, sys.wMonth, sys.wDay, sys.wHour, sys.wMinute, sys.wSecond, sys.wMilliseconds);
		timeStr = buffer;

		// 2021.3.30 20:17:36:341
		outFile.open("data/" + filename + ".txt", std::ios::out | std::ios::app);
		for (int i = 0; i<joints.size(); i++)
		{
			outFile << t <<"," << timeStr << "," << types[joints[i].JointType] << ',' << joints[i].Position.X << ',' << joints[i].Position.Y << ',' << joints[i].Position.Z << std::endl;
		}
		t++;
